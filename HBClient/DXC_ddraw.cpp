...
BOOL DXC_ddraw::bInit(HWND hWnd)
{
 ...
 int            iS, iD;
	...
	m_hFontInUse = NULL;	
	m_hFontInUse = CreateFontA(16,0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, NONANTIALIASED_QUALITY, VARIABLE_PITCH, "Comic Sans MS") ;
	m_hDC = NULL;
...
bool DXC_ddraw::Screenshot(const char* FileName, LPDIRECTDRAWSURFACE7 lpDDS)
{
...
try
{
    ...
    if ((BmpFile = CreateFileA(FileName, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL)) == INVALID_HANDLE_VALUE)
		throw 8;
	...
	OutputDebugStringA("Screenshot Success\r\n");
    Success = true;
}
catch (int &errorcode)
{
    char Buf[100];
    wsprintfA(Buf, "Screenshot error #%i\r\n", errorcode);
