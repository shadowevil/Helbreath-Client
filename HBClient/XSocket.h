...
public:		
	...
	int iSendMsgBlockingMode(const char*buf,int nbytes);
	...
	BOOL bListen(const char* pAddr, int iPort, unsigned int uiMsg);
	
	int iSendMsg(const char* cData, DWORD dwSize, char cKey = NULL);
	BOOL bConnect(const char* pAddr, int iPort, unsigned int uiMsg);
	BOOL bBlockConnect(const char* pAddr, int iPort, unsigned int uiMsg);
