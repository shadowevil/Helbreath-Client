...
void CCurse::filterChar(const char*src, char*dst, const char*chars)
{
	...
	if (strchr(chars, *srcp) == NULL) {
	    *dstp = *srcp;   dstp += 1;
	    ...
	    int msb, lsb;
	    msb = *(unsigned const char*)(srcp-1);
	    lsb = *(unsigned const char*)srcp;
	    if (srcp > src && (
...
{
    unsigned const char*src = (unsigned const char*)buf;
...
char* CCurse::ConvertString(char* str, int max_len)
{
    ...
    const char*ptr = curse_string, *lptr, *cptr;
